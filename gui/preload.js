...
contextBridge.exposeInMainWorld("rmmHunter", {
  ...
  getWatchStatus: () => ipcRenderer.invoke("watch:getStatus"),
  saveWatchSettings: (settings) => ipcRenderer.invoke("watch:saveSettings", settings),
  runWatchOnce: (options) => ipcRenderer.invoke("watch:runOnce", options),
  sendWatchTestAlert: () => ipcRenderer.invoke("watch:testAlert"),
  respondToWatchAlert: (payload) => ipcRenderer.invoke("watch:respond", payload),
